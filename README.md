...
3. The OTel collector deep-merges its base config (NIC's defaults) with this override at startup. Our pipeline overrides replace the `[debug]` exporter lists with `[otlphttp/loki]`, `[otlp/tempo]`, and `[otlphttp/mimir]` and set nothing else, so NIC's receivers and processors (including `k8sattributes`) are kept. The merge replaces lists rather than appending, so restating either list here would drop NIC's entries.
