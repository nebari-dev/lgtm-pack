...
```yaml
  ...
  #
  # The bundled MinIO uses the upstream chart's dev-grade defaults (static
  # credentials rendered into the Mimir config ConfigMap, 5Gi PVC). For
  # production, use the cloud-bucket example below, or at minimum override
  # minio.rootUser, minio.rootPassword, and minio.persistence.size.
  #
  # To use a real cloud bucket instead, disable minio and supply the s3
  # config. The ${...} placeholders are expanded from the container
  # environment (the subchart runs Mimir with -config.expand-env), so the
  # credentials must be injected via global.extraEnvFrom, e.g.:
  # minio:
  #   enabled: false
  # global:
  #   extraEnvFrom:
